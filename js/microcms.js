...
/** Worksページのヒーローリード文（site-settings の worksLead） */
function getWorksLead(settings) {
  const text =
    (settings && settings.worksLead) || FALLBACK_SITE_SETTINGS.worksLead;
...
/** 記事ページURL（scripts/generate-articles.mjs で生成されるHTMLに合わせる） */
function getArticleUrl(post) {
  if (!post || !post.id) return "article.html";
  return `articles/${encodeURIComponent(post.id)}.html`;
}

/** 制作範囲（CMSで scope / score など名前が違う場合の互換） */
function getWorkScope(work) {
  if (!work) return "";
  return work.scope || work.score || "";
}

/** 参考制作費（CMSで priceRange / proceRange など名前が違う場合の互換） */
function getWorkPriceRange(work) {
  if (!work) return "";
  return work.priceRange || work.proceRange || "";
}
